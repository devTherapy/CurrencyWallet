...
namespace CurrencyWallet.Infrastructure.CurrencyConverter
{
    public class CurrencyConverterService : ICurrencyConverterService
    {
        private readonly FixerSettings _FixerSettings;

        public CurrencyConverterService(IOptions<FixerSettings> FixerSettings)
        {
            _FixerSettings = FixerSettings.Value;
        }
        public async Task<decimal> ConvertCurrency(string transactionCurrency, string MainCurrency, decimal amount)
        {
            ...
            var conversion = (transactionCurrencyRate * amount) / MainCurrencyRate;
            return conversion;
        }
        ...
        public  async Task<Rates> GetCurrencyRates()
        {
            ...
            HttpResponseMessage response = await client.GetAsync($"{_FixerSettings.Base_Url}/latest?access_key={_FixerSettings.Key}");
            string content = await response.Content.ReadAsStringAsync();
            ResponseData data = JsonConvert.DeserializeObject<ResponseData>(content);
            return data.rates;
        }
        ...
        public async Task<decimal> SelectCurrencyRate(string currency)
        {
            decimal rate = 0;
